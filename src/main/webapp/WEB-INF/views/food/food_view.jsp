...
<title>댕댕이맛집 글 보기</title>
<link rel="stylesheet" href="resources/css/title.css"/>
<link href="resources/css/food_view.css" rel="stylesheet">
		    <!-- 헤더 부분  끝-->
<script>
function DoDelete(bno)
{
	...
	document.location = "food_delete.do?no=" + bno;
}
...
function ReplyDelete(rno)
{
	...
	$.ajax({
		url : "food_replydeleteok.do?rno=" + rno,
		 type : "get",
		 dataType : "html",
		 success : function(response)
		 {
			...
			<table width="100%" align="center">
				<tr>
					<td style="text-align:center; border:0px" >
						<a href=./food.jsp style="color: black; margin: 20px;"><h1>댕댕이 맛집 게시판</h1></a>
						<hr>
				 	</td>  
				</tr>						
				<tr>
					<td style="border:0px;">
						<table border="0" width="100%" align="center">
							...
							<tr>
								...
								<td colspan="2">
									${ item.tel }-${ item.tel1 }-${item.tel2 }
								</td>
								<td>
									영업시간
								</td>
								<td colspan="2">
									${ item.btime1 }~${item.btime2 }
								</td>
							</tr>
							<tr>
								<td>
							...
							<tr>
								...
								<c:if test="${ sessionScope.login != null }">	
									<c:if test="${ login.userno == item.userno || login.ugrade == 0}">
										<input type="button" value="수정" style="margin:0 0 0 795px; width:70px; height:30px; font-size:20px" onclick="location.href='food_update.do?no=${ item.bno }'">
										<input type="button" value="삭제" style="margin:0 0 0 20px; width:70px; height:30px; font-size:20px" onclick="DoDelete(${ item.bno });">
										<input type="button" value="목록" style="margin:0 0 0 20px; width:70px; height:30px; font-size:20px" onclick="location.href='food.do'">
									</c:if>
								</c:if>
								<c:if test="${ sessionScope.login == null || login.userno != item.userno  }">
									<c:if test="${ login.ugrade != 0 }">
										<input type="button" value="목록" style="margin:0 0 0 835px; width:70px; height:30px; font-size:20px" onclick="location.href='food.do'">
									</c:if>
								</c:if>
								</td>					
							</tr>
